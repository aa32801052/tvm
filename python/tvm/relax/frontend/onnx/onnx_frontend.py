...
def _tensor_to_shape_expr(
    ...
    shape_vars = [tir.Var(f"{prefix}_{i}", "int64") for i in range(shape_ndim)]
...
def _build_squeezed_shape_tensor(
    ...
    num_keep_dims = tir.Var("squeeze_num_keep_dims", "int64")
    ...
    def _impl_v13(cls, bb, inputs, attr, params):
        ...
        if all_constant_params:
            ...
            if any(
                ...
                or (isinstance(step, tir.IntImm) and int(step) == 0)
                for step in steps
            ):
            ...
            for i, s in enumerate(target_shape):
                if isinstance(s, tvm.tir.IntImm):
                    if s.value == -1:
                        ...
                        isinstance(data_shape[i], tvm.tir.IntImm)
...
def _argreduce_select_last_index(bb, data, axis, keepdims, op):
    ...
    if isinstance(axis_size, tir.IntImm):
...
class SplitToSequence(OnnxOpConverter):
    ...
    def _impl_v11(cls, bb, inputs, attr, params):
        ...
        if len(split.shape) == 1 and split.shape[0] > 1:
            ...
            if isinstance(dim_size, (int, tir.IntImm)):
