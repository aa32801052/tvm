...
class CustomDatatypesLowerer : public StmtExprMutator {
 public:
  ...
  Expr VisitExpr_(const CastNode* op) final {
    auto type_code = static_cast<uint8_t>(op->ty.as_or_throw<PrimType>().code());
    auto src_type_code = static_cast<uint8_t>(op->value.ty().code());
  ...
  Expr VisitExpr_(const FloatImmNode* imm) final {
    auto type_code = static_cast<uint8_t>(imm->ty.as_or_throw<PrimType>().code());
  ...
  BufferVar GetRemappedBuffer(BufferVar buf) {
    ...
    bool to_be_lowered = datatype::Registry::Global()->GetTypeRegistered(
        static_cast<uint8_t>(buf->dtype.code()));

    if (to_be_lowered) {
      auto new_load_type = PrimType::UInt(buf->dtype.bits());
      BufferVar new_buf = buf.with_dtype(new_load_type);
  ...
  Expr VisitExpr_(const CallNode* call) final {
    ...
    auto type_code = static_cast<uint8_t>(call->ty.as_or_throw<PrimType>().code());
    bool to_be_lowered = datatype::Registry::Global()->GetTypeRegistered(type_code);
    Expr expr = StmtExprMutator::VisitExpr_(call);
    call = expr.as<CallNode>();
    if (to_be_lowered) {
      auto op = call->op.as<OpNode>();
      TVM_FFI_ICHECK(op != nullptr) << "Lowering non-intrinsic Calls not implemented";
      auto lower = datatype::GetIntrinLowerFunc(target_, op->name, type_code);
...
#define TVM_DEFINE_MUTATE_CUSTOM_DTYPE(OP, NodeName)                                       \
  Expr VisitExpr_(const NodeName* op) final {                                              \
    auto type_code = static_cast<uint8_t>(op->ty.as_or_throw<PrimType>().code());           \
    bool to_be_lowered = datatype::Registry::Global()->GetTypeRegistered(type_code);       \
    Expr expr = StmtExprMutator::VisitExpr_(op);                                           \
    op = expr.as<NodeName>();                                                              \
