...
void quire_matmul(storage_t<bits>* A, int64_t a_offset, int64_t K, storage_t<bits>* B,
                  int64_t b_offset, int64_t column, int64_t N, storage_t<bits>* C,
                  int64_t c_offset) {
  sw::universal::quire<bits, es> accumulator;
  accumulator.clear();

  for (int64_t k = 0; k < K; ++k) {
    auto lhs = from_bits<bits, es>(A[a_offset + k]);
    auto rhs = from_bits<bits, es>(B[b_offset + k * N + column]);
    accumulator += (lhs * rhs).to_value();
  }
...
#define TVM_DEFINE_POSIT_WRAPPERS(bits, Es)                                              \
  ...
  TVM_DLL void Posit##bits##es##Es##QuireMatmul(                                        \
